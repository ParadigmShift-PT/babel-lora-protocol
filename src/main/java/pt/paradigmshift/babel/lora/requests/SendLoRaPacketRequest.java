 ...
 * tagging each request with their own {@code appId} (conventionally the
 * sender's protocol id) — receivers see that same identifier on the inbound
...
public class SendLoRaPacketRequest extends ProtoRequest {
     ...
     */
    public SendLoRaPacketRequest(short appId, int destAddress, byte[] payload) {
        super(REQUEST_ID);
        this.appId = appId;
